...
const INTERNAL_ROLES = INTERNAL_ORG_ROLES;
const EXTERNAL_ROLES = EXTERNAL_ORG_ROLES;
...
export type InviteUserFormProps = {
  /** `modal` = dialog shell; `embedded` = settings workbench right column */
  ...
  /** After successful send when `variant` is `embedded` */
...
export function InviteUserForm({
  ...
  }, [variant, open, prefillFirstName, prefillLastName, prefillEmail]);
  ...
  if (variant === "embedded") {
    return (
      <div className="flex max-h-[min(720px,calc(100vh-9rem))] min-h-0 flex-col overflow-hidden rounded-[10px] bg-card shadow-e1">
        <div className="shrink-0 border-b border-border/25 bg-surface-gradient/40 px-4 py-3 shadow-paper-edge">
          <p className="text-sm font-semibold text-foreground">Invite member</p>
          <p className="text-xs text-muted-foreground mt-0.5">Add someone to your organisation.</p>
        </div>
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-4 py-4">{inviteFormSections}</div>
        <div className="flex shrink-0 justify-end gap-2 border-t border-border/30 bg-card/95 px-4 py-3">
          {footerActions}
        </div>
      </div>
    );
  ...
  return (
    <Dialog open={!!open} onOpenChange={handleDialogOpenChange}>
      <DialogContent className="sm:max-w-[760px] max-h-[90vh] overflow-y-auto p-0 gap-0 rounded-[12px] shadow-e3">
        ...
        <div className="space-y-5 px-6 pb-6">{inviteFormSections}</div>

        <DialogFooter className="border-t border-border/40 px-6 pb-6 pt-2">{footerActions}</DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
