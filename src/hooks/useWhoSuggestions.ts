...
type WhoMember = { user_id: string; display_name: string };
type WhoTeam = { id: string; name: string | null };

export function computeWhoProposals(
  inputValue: string,
  members: WhoMember[],
  teams: WhoTeam[]
): WhoProposal[] {
  ...
  const personExact = members.find((m) => m.display_name.toLowerCase() === lowerInput);
  ...
  const teamExact = teams.find((t) => (t.name || "").toLowerCase() === lowerInput);
...
export function useWhoSuggestions(inputValue: string, enabled: boolean) {
  ...
  return useMemo(
    () => computeWhoProposals(debouncedValue, members, teams),
