-- Building setup assistant V1
-- Bootstraps plan schema if missing (20260315 was recorded applied without tables on Filla-v2),
-- then adds setup context + review-before-create defaults + spaces-only import.
...
WITH CHECK (
  ...
  AND EXISTS (
    ...
    WHERE om.org_id = split_part(name, '/', 2)::uuid
...
-- ============================================================================
-- V1 additions: setup context + review defaults + spaces-only import
-- ============================================================================

ALTER TABLE property_plan_files
  ADD COLUMN IF NOT EXISTS building_label TEXT,
  ADD COLUMN IF NOT EXISTS floor_label TEXT,
  ADD COLUMN IF NOT EXISTS scale_known BOOLEAN,
  ADD COLUMN IF NOT EXISTS units TEXT,
  ADD COLUMN IF NOT EXISTS setup_notes TEXT;

COMMENT ON COLUMN property_plan_files.building_label IS
  'User-confirmed building name for this plan sheet (setup assistant).';
COMMENT ON COLUMN property_plan_files.floor_label IS
  'User-confirmed floor / level for this plan sheet (setup assistant).';

ALTER TABLE extracted_spaces
  ADD COLUMN IF NOT EXISTS floor_label TEXT,
  ADD COLUMN IF NOT EXISTS review_band TEXT;

COMMENT ON COLUMN extracted_spaces.floor_label IS
  'Floor assignment proposed or confirmed for this space suggestion.';
COMMENT ON COLUMN extracted_spaces.review_band IS
  'UI band: reliable | needs_confirmation | incomplete.';

ALTER TABLE extracted_spaces
  ALTER COLUMN is_accepted SET DEFAULT FALSE;

ALTER TABLE extracted_assets
  ALTER COLUMN is_accepted SET DEFAULT FALSE;

ALTER TABLE extracted_compliance_elements
  ALTER COLUMN is_accepted SET DEFAULT FALSE;

ALTER TABLE extracted_task_suggestions
  ALTER COLUMN is_accepted SET DEFAULT FALSE;

CREATE OR REPLACE FUNCTION import_plan_extraction_run(
  p_extraction_run_id UUID,
  p_spaces_only BOOLEAN DEFAULT TRUE
)
RETURNS TABLE(
  created_spaces INTEGER,
  created_assets INTEGER,
...
REVOKE ALL ON FUNCTION import_plan_extraction_run(UUID, BOOLEAN) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION import_plan_extraction_run(UUID, BOOLEAN) TO authenticated;

CREATE OR REPLACE FUNCTION import_plan_extraction_run(p_extraction_run_id UUID)
RETURNS TABLE(
  created_spaces INTEGER,
  created_assets INTEGER,
  created_tasks INTEGER
