-- Remote Filla-v2 is missing image-pipeline columns on attachments even though
-- older migrations are marked applied. Create-task image inserts fail without them
-- (optimized_url / annotation_json / upload_status), so task cards never get thumbs.
...
CREATE INDEX IF NOT EXISTS idx_attachments_upload_status
  ON public.attachments (upload_status)
  WHERE upload_status IS DISTINCT FROM 'complete';

-- Recreate tasks_view so images can surface optimized_url when present.
...
COMMENT ON VIEW public.tasks_view IS
  'Tasks with relationships. images aggregates attachments (thumbnail preferred); owner_user_id is From, assigned_user_id is For.';
