...
*   `ai_batch_jobs` — durable Gemini Batch jobs (half-price overnight). **Platform scope, not org data** (sentinel `org_id`). Columns: `capability` (`knowledge_guidance_draft` | `knowledge_gap_research` | `content_seo_draft` | `content_brief_draft` | `content_output_draft` | `content_visual_brief`), `mode`, `status` (`queued` | `submitted` | `running` | `intake_pending` | `succeeded` | `failed` | `cancelled`), `created_by`, `provider` / `model_used` / `prompt_version`, `provider_batch_id`, `item_ids` jsonb, counts, `error_message`, `metadata`, timestamps. RLS: `is_platform_admin()` SELECT only; no client INSERT/UPDATE. Edge Functions `ai-batch-submit` / `ai-batch-poll` write with service role **after** a `platform_admins` check (`verify_jwt = true`). `admin_list_ai_batch_jobs` is the named read (Ch 25). `apply_ai_batch_guidance_result` is **service_role only** and writes unverified drafts for rows in that job — never verify/publish. Content capabilities are in the CHECK so Outputs can share the table; submit returns `501 capability_not_enabled` until `content-generate` grows a batch processor. Cap: 80 guidance items / 20 research gaps per job. Schedule `ai-batch-poll` about every 15 minutes (service-role bearer) in addition to the Knowledge page poll.
...
*   `knowledge` — `id`, `scope`, `status`, `org_id` (NULL iff `scope = platform`; CHECK: platform ⇒ `org_id` NULL, organisation ⇒ `org_id` NOT NULL), `title`, `summary`, `body`, `content` (jsonb — extractor/discovery blobs), `attributes` (jsonb object — type-specific structured facts from intake; freeform keys, not fixed columns), `source_kind`, `trust_score` (critic-written numeric), `provenance` (jsonb: extractor/critic models, prompt versions, source refs, intake sheet/row), `cohort_size` (nullable; required ≥ `BRAIN_MIN_COHORT` = 5 before publish when `source_kind = community_brain`), `version`, `supersedes_id` (self-FK), `created_by`, `reviewed_by`, `published_at`, `applicability` (jsonb — see below), timestamps. Title/summary remain the concise representation; detailed facts live in `knowledge_claims`.
*   **Applicability (required for Intake creates):** `knowledge.applicability` jsonb, normalised shape `{ jurisdictions: string[], regions: string[], languages: string[], audiences: ("owner"|"manager"|"field"|"tenant"|"public")[], unscoped?: boolean }`. Empty jurisdictions are allowed only when `unscoped = true` (explicit platform-global). Normalised via `normalize_knowledge_applicability`.
*   `knowledge_claims` — atomic source-backed facts (or explicit unknowns) under a Knowledge item: `claim_text`, `category` (`obligation` | `applicability` | `responsibility` | `standard` | `testing` | `replacement` | `evidence` | `exception` | `consequence` | `unknown` | `other`), optional claim `applicability`, `source_id` → `knowledge_sources`, `source_location`, `verification_status` (`extracted` | `verified` | `unresolved` | `unknown` | `rejected`), `confidence`, `critic_result`, `reviewed_by`/`reviewed_at`, `sort_order`. Extractor must not invent facts; missing detail is `unknown`. Topic human verify promotes `extracted` → `verified` and leaves `unknown`/`unresolved` intact. RLS mirrors `knowledge_sources` (org members / published platform SELECT; org Owner/Manager write).
*   `knowledge_sources` — citations / attachments / URLs / brain pattern keys backing a knowledge row (written on Intake; `metadata` may hold `storage_bucket` / `storage_path` for platform intake files). Platform admins may edit URL/label via `admin_update_knowledge_source` (invalidates critic).
*   `knowledge_intake_batches` — platform-admin bulk Upload provenance (filename, storage path, column mapping, created counts). RLS: platform admin only.
...
*   **Content Tree (internal editorial, platform admin only):** `content_topics` (FK `knowledge_id` to verified|published platform Knowledge; `workflow_status` sequential stage gate; `seo` / `brief` / `creative` / `publishing` jsonb stage envelopes with version history; `applicability_snapshot`; `upstream_hash`) and `content_outputs` (`output_kind`: `core_article` | `faq` | `in_app_tip` | `newsletter` | `social_post` | `reel_script`; created on demand after brief approval; `status`: draft|needs_review|approved|rejected|needs_update|archived). SEO/brief/output generation receives **verified** `knowledge_claims` only and treats unknown/unresolved claims as Knowledge gaps (`evidence_gaps`) — content must not invent missing facts. After each output draft, `content_output_grounding` critic compares factual assertions to verified claims and stores the result in `content_outputs.structured.grounding` / provenance. SEO approval (`admin_approve_content_topic_seo`) is refused when `source_content_unavailable` is true or material `evidence_gaps` / `research_warnings` remain — ungrounded proposals cannot advance. Approved output bodies are not silently overwritten. Upstream Knowledge changes mark stage envelopes stale and outputs `needs_update` without clearing body.
*   **RLS:** Organisation rows — members SELECT; Owner/Manager INSERT/UPDATE candidates and verified; publish/archive Owner/Manager only; customers never UPDATE `scope = platform`. Platform `published` — SELECT for any authenticated org member. Platform non-published — no direct client SELECT; platform_admins via SECURITY DEFINER RPCs only. Service role writes candidates from edges. Content Tree + intake batches — platform admin only.
...
*   **Admin RPCs (intake / detail / content):** `admin_get_knowledge_detail`, `admin_add_knowledge_source`, `admin_update_knowledge_source`, `admin_create_knowledge_intake_batch`, `admin_bulk_create_platform_knowledge_candidates`, `admin_list_content_topics`, `admin_get_content_topic`, `admin_create_content_topic`, `admin_upsert_content_topic_stage`, `admin_set_content_topic_workflow_status`, `admin_approve_content_topic_seo`, `admin_reject_content_topic_seo`, `admin_approve_content_topic_brief`, `admin_reject_content_topic_brief`, `admin_upsert_content_output`, `admin_set_content_output_status`, `admin_list_ai_batch_jobs` (plus existing queue/status/upsert/metrics). Edge: `content-generate` (SEO, brief, outputs, visual concept); `ai-batch-submit` / `ai-batch-poll` (overnight Gemini Batch for Knowledge guidance, gap research, and later Content).
*   **Service helpers:** `create_knowledge_candidate` (optional `p_applicability`), `apply_knowledge_critic_result`, `list_brain_patterns_for_community` (cohort-gated Brain read for Community candidates).
