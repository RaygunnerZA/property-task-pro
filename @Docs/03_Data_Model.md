...
*   `knowledge_sources` — citations / attachments / URLs / brain pattern keys backing a knowledge row (written on Intake; `metadata` may hold `storage_bucket` / `storage_path` for platform intake files).
...
*   **Admin RPCs (intake / detail / content):** `admin_get_knowledge_detail`, `admin_add_knowledge_source`, `admin_create_knowledge_intake_batch`, `admin_bulk_create_platform_knowledge_candidates`, `admin_list_content_topics`, `admin_get_content_topic`, `admin_create_content_topic`, `admin_upsert_content_topic_stage`, `admin_set_content_topic_workflow_status`, `admin_approve_content_topic_seo`, `admin_reject_content_topic_seo`, `admin_approve_content_topic_brief`, `admin_reject_content_topic_brief`, `admin_upsert_content_output`, `admin_set_content_output_status`, `admin_list_ai_batch_jobs` (plus existing queue/status/upsert/metrics). Edge: `content-generate` (SEO, brief, outputs, visual concept); `ai-batch-submit` / `ai-batch-poll` (overnight Gemini Batch for Knowledge guidance, gap research, and later Content).
