...
*   **Content Tree (internal editorial, platform admin only):** `content_topics` (FK `knowledge_id` to verified|published platform Knowledge; `seo` / `brief` / `creative` / `publishing` jsonb stages; `applicability_snapshot`; `upstream_hash`) and `content_outputs` (`output_kind`: `core_article` | `faq` | `in_app_tip`; `status`: draft|needs_review|approved|rejected|needs_update|archived). Approved output bodies are not silently overwritten. Upstream Knowledge / SEO / Brief changes mark outputs `needs_update` without clearing body.
...
*   **Admin RPCs (intake / detail / content):** `admin_get_knowledge_detail`, `admin_add_knowledge_source`, `admin_create_knowledge_intake_batch`, `admin_bulk_create_platform_knowledge_candidates`, `admin_list_content_topics`, `admin_get_content_topic`, `admin_create_content_topic`, `admin_upsert_content_topic_stage`, `admin_upsert_content_output`, `admin_set_content_output_status` (plus existing queue/status/upsert/metrics).
