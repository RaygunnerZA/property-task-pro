...
Watch stores per accepted page: canonical path and GOV.UK content ID, public update timestamp, relevant-section hashes, extracted claims and passage references, affected Knowledge IDs. A scheduled check compares metadata first and analyses the body only when a meaningful change is detected. Initial England watchlist is six families: landlord duties and renting; building regulations and Approved Documents; planning and permitted development; EPC and energy standards; HSE property-safety guidance; Building Safety Regulator guidance. **Scotland:** one Official Source Catalogue entry for the [Building Standards collection](https://www.gov.scot/collections/building-standards/) — current domestic/non-domestic handbooks, supporting guidance (Sections 0–7), procedural/enforcement guidance, and legislation are watched actively; consultations are Potential changes; superseded handbooks and research are indexed only. Applicable Scottish guidance depends on the building-warrant application date (or work commencement when no warrant) — stored as `applicability.applies_when` with edition/building_scope. Add sections only when genuine Knowledge gaps remain.
